...
namespace Unity.VisualScripting.Interpreter
{
    ...
    internal class TypeHash
    {
        ...
        private static Dictionary<Type, ulong> s_Cache;
        private static Dictionary<ulong, Type> s_RevCache;
        ...
        {
            if (s_Cache != null && s_Cache.TryGetValue(type, out var hash))
                return hash;
            ulong versionHash = HashVersionAttribute(type);
            ulong typeHash = HashType(type, null);

            return CombineFNV1A64(versionHash, typeHash);
        }

        public static ulong CacheStableTypeHash<T>() => CacheStableTypeHash(typeof(T));
        public static ulong CacheStableTypeHash(Type type)
        {
            if (s_Cache != null && s_Cache.TryGetValue(type, out var hash))
                return hash;
            ulong versionHash = HashVersionAttribute(type);
            ...
            hash = CombineFNV1A64(versionHash, typeHash);
            AddTypeHashToCache(type, hash);
            return hash;
        }

        internal static void AddTypeHashToCache(Type type, ulong hash)
        {
            if (s_Cache == null)
            {
                s_Cache = new Dictionary<Type, ulong>();
                s_RevCache = new Dictionary<ulong, Type>();
            }
            s_Cache.Add(type, hash);
            s_RevCache.Add(hash, type);
        }

        public static Type GetType(ulong hash) =>
            s_RevCache == null ? null : s_RevCache.TryGetValue(hash, out var t) ? t : null;
